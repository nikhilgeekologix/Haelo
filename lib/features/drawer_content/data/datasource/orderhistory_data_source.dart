import 'package:haelo_flutter/features/drawer_content/data/model/orderhistory_model.dart';
import '../../../../core/utils/exceptions.dart';
import '../../../../core/utils/network_info.dart';
import '../../../../services/network_service.dart';
import '../../../../urls.dart';

class OrderHistoryDataSource {
  final NetworkService networkService;
  final NetworkInfo networkInfo;

  OrderHistoryDataSource(this.networkService, this.networkInfo);

  Future<OrderHistoryModel> fetchOrderHistory(Map<String, String> body) async {
    try {
      final response = await networkService.postRequestNew(
        isFullURL: false,
        url: Urls.ORDER_HISTORY,
        isAuth: true,
        body: body,
        // file: file,
        // fileKey: "uploadFile",
        // versionName: "2.0",
      );
      return OrderHistoryModel.fromJson(response);
    } catch (e) {
      throw ServerException('Failed to get data');
    }
  }
}
